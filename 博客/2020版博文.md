@[toc]
#### 一、项目简介地址
首先节假日定义：[周末 + 节假日 -（被调成工作日的日期）= 节假日]
本项目主要功能是获取全年的节假日，获取2019年和2020年节假日的工具类，网上第三方获取节假日的接口api不稳定，经常失效，建议落地到本地数据库去使用。原因较多
- 避免第三方接口失效或者被改动
- 落地到数据库之后调用速度快
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/20200623144040653.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3UwMTE0NTYzMzc=,size_16,color_FFFFFF,t_70)
#### 三、节假日数据获取
...
5、查看生成的SQL
```
数据库插入的数据可以使用Navicate 。通过查询select * from no_work_day 得到结果集，结果集全选右键复制为insert语句则可以导出所有插入语句。
```
